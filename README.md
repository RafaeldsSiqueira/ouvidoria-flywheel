...
1. **Fila de Auditoria Especializada:** Uma interface administrativa simples (construída em Streamlit ou React) consulta o Firestore e lista chamados com status `AGUARDANDO_HUMANO` e o respectivo gatilho detectado, fornecendo aos analistas jurídicos uma fila de trabalho priorizada.
2. **Data Flywheel:** As correções dos analistas geram registros na coleção `logs_treinamento`. Este histórico estruturado é exportado mensalmente para servir como dataset de *fine-tuning* (ajuste fino) de modelos menores e mais econômicos, ou para recalibrar o prompt do Gemini.
